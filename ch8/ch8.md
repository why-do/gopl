...
```
现在基本就避免了在取消后创建新的 goroutine。但是其他已经创建的 goroutine 则会等待他们执行完毕。要想更快的响应，就需要更多的程序逻辑变更入侵。要确保在取消事件之后没有更多昂贵的操作发生。这就需要更新更多的代码，但是通常可以通过在少量重要的地方检察取消装来来达到目的。在 dirents 中获取信号量令牌的操作也可需要快速结束：
```go
func dirents(dir string) []os.FileInfo {
	select {
